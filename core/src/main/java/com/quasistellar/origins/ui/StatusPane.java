...
import com.quasistellar.origins.windows.WndGame;
import com.quasistellar.origins.windows.WndHero;
import com.quasistellar.origins.windows.WndJournal;
...
public class StatusPane extends Component {
	...
	protected void layout() {
		...
		bossHP.setPos( 6 + (width - bossHP.width())/2, 25);
	...
	public void update() {
		...
		float max = Math.max(Dungeon.hero.HT, Dungeon.hero.HP + Dungeon.hero.SHLD);

		float spirit = Dungeon.hero.SP;
		float max_spirit = Dungeon.hero.SM;
		...
		shieldedHP.scale.x = (health+shield)/max;
		rawShielding.scale.x = shield/max;

		sp.scale.x = Math.max( 0, spirit/max_spirit);
