...
public enum Icons {
	...
	public static Image get( Icons type ) {
		...
		case RESUME:
			...
			break;
		case C_SKILL:
			icon.frame( icon.texture.uvRect( 32, 64, 44, 78 ) );
			break;
		case E_SKILL:
			icon.frame( icon.texture.uvRect( 48, 64, 62, 80 ) );
			break;
		case W_SKILL:
			icon.frame( icon.texture.uvRect( 64, 64, 76, 80 ) );
			break;
		case C_ABILITY:
			icon.frame( icon.texture.uvRect( 80, 64, 96, 78 ) );
			break;
		case E_ABILITY:
			icon.frame( icon.texture.uvRect( 96, 64, 108, 79 ) );
			break;
		case W_ABILITY:
			icon.frame( icon.texture.uvRect( 112, 64, 123, 80 ) );
			break;
		}
		return icon;
	}
	...
	public static Image get( HeroClass cl ) {
		switch (cl) {
		case CHEVALIER:
			return get( CHEVALIER );
		case ENCHANTER:
			return get( ENCHANTER );
		case WANDERER:
			return get( WANDERER );
		default:
			return null;
