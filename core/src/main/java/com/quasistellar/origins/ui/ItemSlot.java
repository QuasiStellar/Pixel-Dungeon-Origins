...
public class ItemSlot extends Button {
	...
	private void updateText(){
		...
		if (item instanceof Key && !(item instanceof SkeletonKey)) {
		...
		if (item instanceof Scroll || item instanceof Potion) {
			...
			if (!(item instanceof Staff)) {
				bottomRightIcon = new Image(Assets.CONS_ICONS);
				int left = Math.min(((Weapon) item).durability / 10, 5) * 7;
				int top = 16;
				bottomRightIcon.frame(left, top, 7, 8);
				add(bottomRightIcon);

				if (((MeleeWeapon) item).level != 0) {
					topLeft.text(Messages.format(TXT_LEVEL, ((MeleeWeapon) item).level));
					topLeft.measure();
					topLeft.hardlight(((MeleeWeapon) item).level > 0 ? UPGRADED : DEGRADED);
				}
			}
		} else {
			bottomRight.text( null );
