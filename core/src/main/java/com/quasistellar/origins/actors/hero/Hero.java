...
import com.quasistellar.origins.Bones;
import com.quasistellar.origins.Dungeon;
import com.quasistellar.origins.GamesInProgress;
...
import com.quasistellar.origins.actors.buffs.Awareness;
import com.quasistellar.origins.actors.buffs.Barkskin;
import com.quasistellar.origins.actors.buffs.Berserk;
...
import com.quasistellar.origins.items.Heap.Type;
...
import com.quasistellar.origins.items.keys.CrystalKey;
...
import com.quasistellar.origins.levels.features.Chasm;
...
import com.quasistellar.origins.scenes.InterlevelScene;
import com.quasistellar.origins.scenes.SurfaceScene;
import com.quasistellar.origins.sprites.CharSprite;
import com.quasistellar.origins.sprites.HeroSprite;
import com.quasistellar.origins.ui.AttackIndicator;
import com.quasistellar.origins.ui.BuffIndicator;
import com.quasistellar.origins.ui.RushIndicator;
import com.quasistellar.origins.ui.QuickSlotButton;
import com.quasistellar.origins.utils.BArray;
import com.quasistellar.origins.utils.GLog;
import com.quasistellar.origins.windows.WndAlchemy;
import com.quasistellar.origins.windows.WndForging;
import com.quasistellar.origins.windows.WndMessage;
import com.quasistellar.origins.windows.WndResurrect;
import com.quasistellar.origins.windows.WndTradeItem;
...
public class Hero extends Char {
	...
	private static final float SPIRIT_FOR_SEARCH	= 3f;
	
	public HeroClass heroClass = HeroClass.CHEVALIER;
	...
	public void updateSM( boolean boostSP ){
		int curSM = SM;

		SM = 20 + 2*(lvl-1);

		if (boostSP){
			SP += Math.max(SM - curSM, 0);
		}
		SP = Math.min(SP, SM);
	}
	...
	public void live() {
		...
		Buff.affect( this, Fatigue.class );
	}
	...
	public boolean shootPolearm( Char enemy, Weapon wep ) {
	...
	public boolean act() {
		...
		if (curAction == null) {
			...
			if (resting) {
				...
				buff(Fatigue.class).reduceSpirit(-0.2f);
				if (this.SP == this.SM) resting = false;
				next();
				return false;
			}
			...
			if (curAction instanceof HeroAction.Forging) {

				return actForging( (HeroAction.Forging)curAction );
	...
	private boolean actForging( HeroAction.Forging action ) {
		int dst = action.dst;
		if (Dungeon.level.distance(dst, pos) <= 1) {

			ready();
			GameScene.show(new WndForging());
			return false;
	...
	public int attackProc( Char enemy, int damage ) {
		...
		case CHEVALIER:
			if (enemy instanceof Mob) {
				if (!((Mob)enemy).enemySeen && canSurpriseAttack()) {
					damage *= 2;
				}
			}
		}
	...
	public void damage( int dmg, Object src ) {
		...
		if (!(src instanceof Hunger || src instanceof Fatigue || src instanceof Viscosity.DeferedDamage) && damageInterrupt) {
			interrupt();
			resting = false;
		}
	...
	private boolean getCloser( final int target ) {
		...
		if (step != -1) {
			...
			if (subClass == HeroSubClass.EXPLORER){
	...
	public boolean handle( int cell ) {
		...
		} else if (Dungeon.level.map[cell] == Terrain.ANVIL && cell != pos) {

			curAction = new HeroAction.Forging(cell);
	...
	public boolean isTired() {
		return buff(Fatigue.class) != null && ((Fatigue)buff( Fatigue.class )).isTired();
	}

	public boolean isRested() {
		return buff(Fatigue.class) != null && ((Fatigue)buff( Fatigue.class )).isRested();
	}
	...
	public void die( Object cause  ) {
		...
		if (Dungeon.hero.belongings.weapon != null && Dungeon.hero.belongings.weapon.hasStone(new StoneOfSacrifice())) {
			this.HP = HT/4;
	...
	public boolean search( boolean intentional ) {
		...
		int distance = heroClass == HeroClass.WANDERER ? 2 : 1;
				...
				if (fieldOfView[p] && p != pos) {
					...
					if (Dungeon.level.secret[p]){
						...
						//unintentional trap detection scales from 40% at floor 0 to 30% at floor 25
							...
							chance = Dungeon.hero.isRested() ? 0.6f - (Dungeon.depth / 166.7f) : 0.4f - (Dungeon.depth / 250f);
							
						//unintentional door detection scales from 20% at floor 0 to 0% at floor 20
						} else {
							chance = Dungeon.hero.isRested() ? 0.3f - (Dungeon.depth / 66.7f) : 0.2f - (Dungeon.depth / 100f);
						}
		...
		if (intentional) {
			...
			buff(Fatigue.class).reduceSpirit(SPIRIT_FOR_SEARCH);
