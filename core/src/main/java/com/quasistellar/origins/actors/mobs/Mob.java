...
import com.quasistellar.origins.effects.Surprise;
...
public abstract class Mob extends Char {
		...
		if (Dungeon.hero.isAlive()) {
			...
			if (alignment == Alignment.ENEMY) {
				...
				if (exp > 0) {
					...
					if (Dungeon.hero.belongings.weapon != null && Dungeon.hero.belongings.weapon.hasStone(new StoneOfCruelty())) {
	...
	public void rollToDropLoot(){
		...
		if (Dungeon.hero.belongings.weapon != null && Dungeon.hero.belongings.weapon.hasStone(new StoneOfGreed())) {
			lootChance *= 1.2f;
		}
