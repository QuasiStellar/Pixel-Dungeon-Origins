...
public class Ballistica {
	...
	public Ballistica( int from, int to, int params ){
		sourcePos = from-1;
		build(from-1, to, (params & STOP_TARGET) > 0, (params & STOP_CHARS) > 0, (params & STOP_TERRAIN) > 0);
		if (collisionPos != to || Dungeon.level.losBlocking[from-1]) {
			collisionPos = null;
			path = new ArrayList<>();
			sourcePos = from+1;
			build(from+1, to, (params & STOP_TARGET) > 0, (params & STOP_CHARS) > 0, (params & STOP_TERRAIN) > 0);
			if (collisionPos != to || Dungeon.level.losBlocking[from+1]) {
				collisionPos = null;
				path = new ArrayList<>();
				sourcePos = from-Dungeon.level.width();
				build(from-Dungeon.level.width(), to, (params & STOP_TARGET) > 0, (params & STOP_CHARS) > 0, (params & STOP_TERRAIN) > 0);
				if (collisionPos != to || Dungeon.level.losBlocking[from-Dungeon.level.width()]) {
					collisionPos = null;
					path = new ArrayList<>();
					sourcePos = from+Dungeon.level.width();
					build(from+Dungeon.level.width(), to, (params & STOP_TARGET) > 0, (params & STOP_CHARS) > 0, (params & STOP_TERRAIN) > 0);
