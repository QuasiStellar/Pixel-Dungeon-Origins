...
import com.quasistellar.origins.Rankings;
import com.quasistellar.origins.SPDSettings;
import com.quasistellar.origins.Origins;
import com.quasistellar.origins.effects.BannerSprites;
import com.quasistellar.origins.effects.Fireball;
...
public class WelcomeScene extends PixelScene {
	...
	private static int LATEST_UPDATE = Origins.v0_1;
	...
	public void create() {
		...
		if (Origins.versionCode == previousVersion) {
		...
		Image title = new Image( BannerSprites.get( BannerSprites.Type.PIXEL_DUNGEON ) ) {
			private float time = 0;
			@Override
			public void update() {
				super.update();
				am = Math.max(0.5f, 0.5f - (float)Math.sin( time += Game.elapsed ) / 2);
		...
		DarkRedButton okay = new DarkRedButton(Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				Origins.switchScene(TitleScene.class);
			}
		};
		...
		if (previousVersion != 0){
			DarkRedButton changes = new DarkRedButton(Messages.get(this, "changelist")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					Origins.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= Origins.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion <= Origins.v0_6_2e){
		...
		if (previousVersion <= Origins.v0_6_0b){
		...
		SPDSettings.version(Origins.versionCode);
	}
