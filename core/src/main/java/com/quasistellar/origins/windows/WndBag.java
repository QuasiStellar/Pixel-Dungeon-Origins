...
import com.quasistellar.origins.items.Gold;
...
import com.quasistellar.origins.items.trash.Trash;
...
import com.quasistellar.origins.plants.Plant.Seed;
...
public class WndBag extends WndTabbed {
	...
	public WndBag( Bag bag, Listener listener, Mode mode, String title ) {
		...
		//nRows = (int)Math.ceil((Belongings.BACKPACK_SIZE + 5) / (float)nCols);
		nRows = (int)Math.ceil((item_quantity + 5) / (float)nCols);
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.STONE && (item instanceof Runestone) ||
						mode == Mode.FORGING && (item instanceof MeleeWeapon && !item.isEquipped( Dungeon.hero ) || item instanceof Trash) ||
						mode == Mode.ALL
					);
			...
			if (!lastBag.contains(item) && !item.isEquipped(Dungeon.hero)){
				...
				if (item instanceof MeleeWeapon) {
					if (item.isIdentified()) {
						GameScene.show(new WndMeleeWeapon( WndBag.this, item ) );
					} else {
